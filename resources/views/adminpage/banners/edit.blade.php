                    ...
                    <div class="card mb-4">
                        ...
                        <form action="{{ route('admin.banners.update', $banner->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-3">
                                    <label for="image" class="form-label">Gambar <small class="text-muted">(Biarkan kosong jika tidak ingin mengubah)</small></label>
                                    ...
                                    <div class="mt-2">
                                        @if($banner->image)
                                            <img id="image-preview" src="{{ asset('storage/' . $banner->image) }}" alt="Preview" style="max-height: 200px; display: block; border-radius: 4px;">
                                        @else
                                ...
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <select class="form-control" id="type" name="type" required>
                                            <option value="slider_left" {{ old('type', $banner->type) == 'slider_left' ? 'selected' : '' }}>Slider Kiri (Besar)</option>
                                            <option value="slider_right" {{ old('type', $banner->type) == 'slider_right' ? 'selected' : '' }}>Slider Kanan (Kecil)</option>
                                        </select>
                                        @error('type') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <label for="position" class="form-label">Posisi Urutan <span class="text-danger">*</span></label>
                                        <input type="number" class="form-control @error('position') is-invalid @enderror" id="position" name="position" value="{{ old('position', $banner->position) }}" min="0" required>
...
<script>
    function previewImage() {
        ...
        if (image.files && image.files[0]) {
            ...
            // Keep original image if no new file is selected
